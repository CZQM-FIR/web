<script lang="ts">
  import type { Snippet } from 'svelte';
  import type { LayoutData } from './$types';
  import Icon from '@iconify/svelte';
  import NavLink from './NavLink.svelte';

  let { data, children }: { data: LayoutData; children: Snippet } = $props();

  function getGreeting(): string {
    const currentHour = new Date().getHours();
    if (currentHour < 12) {
      return 'Good Morning';
    } else if (currentHour < 18) {
      return 'Good Afternoon';
    } else {
      return 'Good Evening';
    }
  }
</script>

<section id="my" class="">
  <div class="container mx-auto min-h-screen">
    <h1 class="pt-6 text-2xl">{getGreeting()}, Jack!</h1>
    <div class="divider"></div>
    <div class="flex flex-row gap-3">
      <!-- sidebar -->
      <div class="w- flex min-h-[30rem] min-w-56 flex-col rounded-2xl bg-base-300 p-2">
        <h3 class="m-0 p-0 text-center text-lg font-semibold">MyCZQM</h3>
        <div class="divider m-0 p-0"></div>
        <ul class="mt-1 ps-2">
          <NavLink name="My Profile" icon="mdi:user-outline" href="/my" />
          <NavLink name="Training" icon="mdi:learn-outline" href="/my/training" />
          <NavLink name="Preferences" icon="mdi:settings-outline" href="/my/preferences" />
          <NavLink name="Logout" icon="mdi:logout" href="/auth/logout" />
        </ul>
      </div>
      <!-- content -->
      <div>
        {@render children()}
      </div>
    </div>
  </div>
</section>
